...
interface BarData {
  month: string;
  "Total Loan Requests": number;
  "Loan's Pending": number;
  "Loan's Rejected": number;
  "Loan's Disbursed": number;
}
...
const TotalLoansComp = () => {
  ...
  useEffect(() => {
    ...
    for (let step = 0; step < months.length; step++) {
      const totalLoans = faker.helpers.rangeToNumber({
        min: 500,
        max: 2000,
      });
      const pendingLoans = faker.helpers.rangeToNumber({
        min: 100,
        max: 1000,
      });
      ...
      const disbursedLoans = faker.helpers.rangeToNumber({
        min: 150,
        max: 1000,
      });
      ...
      barData.push({
        month: months[step],
        "Total Loan Requests": totalLoans,
        "Loan's Pending": pendingLoans,
        "Loan's Rejected": rejectedLoans,
        "Loan's Disbursed": disbursedLoans,
      });
    ...
    for (let step = 0; step < 100; step++) {
      ...
      const loanstatus = faker.helpers.arrayElement([
        "Pending",
        "Approved",
        "Declined",
        "Confirmed",
        ...
        <div className="relative mt-5">
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={barInfo}>
              ...
              />
              <Bar dataKey="Total Loan Requests" fill="#0096EA" />
              <Bar dataKey="Loan's Pending" fill="#FFC000" />
              <Bar dataKey="Loan's Rejected" fill="#FF0000" />
              <Bar dataKey="Loan's Disbursed" fill="#00B050" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
